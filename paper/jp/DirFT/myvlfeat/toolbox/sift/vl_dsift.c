...
{"NormType",         1,   opt_normtype         },
{"DirichletEps",     1,   opt_dirichleteps     },
{0,                  0,   0                    }
} ;

/* normalization types */
char* normtypes[] = {"L1 normalization", "L2-hysteresis normalization", "Dirichlet-based transform"} ;
            ...
            int nin, const mxArray *in[])
{
  ...
  while ((opt = vlmxNextOption (in, nin, options, &next, &optarg)) >= 0) {
    switch (opt) {
      ...
      case opt_normtype :
        if (!vlmxIsPlainScalar(optarg) || (NormType = (int)*mxGetPr(optarg)) < 0 || NormType > 3) {
          vlmxError(vlmxErrInvalidArgument,"NORMTYPE is not a scalar or it is out of [0,1,2,3].") ;
        }
        break ;

      case opt_dirichleteps :
        if (!vlmxIsPlainScalar(optarg) || (DirEpsilon = (float)*mxGetPr(optarg)) <= 0) {
          vlmxError(vlmxErrInvalidArgument,"DIRICHLETEPS is not a scalar or it is less than or equal to zero.") ;
        }
        break ;
   ...
   * -------------------------------------------------------------- */
  {
    ...
    vl_dsift_set_normalization_type(dsift, NormType);
    if (NormType == 2) vl_dsift_set_dirichlet_eps(dsift, DirEpsilon) ;
    ...
    if (verbose) {
      ...
      NormType = vl_dsift_get_normalization_type(dsift) ;
      ...
      mexPrintf("vl_dsift: normalization type:  %s\n", normtypes[NormType]) ;
      if (NormType == 2){
        mexPrintf("vl_dsift: dirichlet epsilon:    %f\n", vl_dsift_get_dirichlet_eps(dsift)) ;
        mexPrintf("vl_dsift: dirichlet bias:       %f\n", vl_dsift_get_dirichlet_bias(dsift)) ;
      }
    }
